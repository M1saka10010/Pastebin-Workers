...
async function handleRequest(event) {
    ...
    if (!id) {
        ...
        return new Response(render(index), init);
    }
    switch (id) {
        case "create-api":
            const json = await readRequestBody(event.request);
            if (json == 0) {
                return new Response(render("你想干什么？"), init);
            }
        ...
        default:
            ...
            if (!t) {
                return new Response(render(id + ' is empty'), init);
            } else {
                return new Response(render(t), init);
            }
            break;
    }
}
